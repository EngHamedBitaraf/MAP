...
{
    // Init tree
    _rootTreeItem = new DataRecord(tr("Scene"), nullptr);
...
void  DataTree::saveNodeSlot()
{
    ...
    if (itemList.size() == 1)
    {
        ...
        {
            QMessageBox::critical(nullptr, tr("Error"), tr("Failed to save layer"));
        }
        else
        {
            QMessageBox::information(nullptr, tr("Result"), tr("Successfully saved"));
        }
    }
...
bool  DataTree::saveNode(const QString &nodeName)
{
    ...
    QString  path = QFileDialog::getSaveFileName(nullptr, tr("Choose save location"), " ",
