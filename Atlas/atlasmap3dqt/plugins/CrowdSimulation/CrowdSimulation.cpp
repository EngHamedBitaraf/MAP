...
inline osg::ref_ptr<osg::Geode>  arrayToNode(osg::ref_ptr<osg::Vec3Array> array)
{
    ...
    geom->setVertexArray(static_cast<osg::Vec3Array *>(array->clone(osg::CopyOp::DEEP_COPY_ALL)));
...
{
    return static_cast<osg::Vec3Array *>(node->asGeode()->getDrawable(0)->asGeometry()->getVertexArray());
}

CrowdSimulation::CrowdSimulation():
    _numObstacle(0), _numAgentGroup(0), _numPath(0)
{
...
{
    QString  loadPath = QFileDialog::getOpenFileName(nullptr, tr("Open file"), "",
    ...
    if (!nodeFile)
    {
        QMessageBox::warning(nullptr, tr("Error"), tr("Loading failed"));
    ...
    {
        QMessageBox::critical(nullptr, tr("Error"), tr("Invalid setting record."));
...
{
    QString  savePath = QFileDialog::getSaveFileName(nullptr, tr("Save file"), "",
    ...
    while (!writeResult)
    {
        ...
        if (!writeResult)
        {
            int  result = QMessageBox::warning(nullptr, tr("Error"), tr("Save failed, specify another position?"),
...
void  CrowdSimulation::toggle(bool checked)
{
    if (checked)
    {
        ...
        int  mode = QMessageBox::question(nullptr, tr("Choose mode."), tr("Load an existing setting?"), QDialogButtonBox::Yes, QDialogButtonBox::No);
